<template>
  <div>
        ...
        <el-table :data="Data.routerList" border stripe>
          ...
          <el-table-column label="操作">
            <template scope="scope">
              <el-button
                type="success"
                :disabled="Data.currentLevel >= 3"
                round
                @click="toNextLevel(scope.row.routeId)"
                >查看下级</el-button
              >
              <a
                href="javascript:;"
    ...
    >
      <el-form ref="addRouterFormRef" :model="addRouterForm" label-width="70px">
        ...
        <el-form-item label="排序" prop="sortMark">
          <el-input
            v-model.number="addRouterForm.sortMark"
            placeholder="请输入数字"
          ></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="addDialogVisible = false" icon="el-icon-error"
          >取 消</el-button
        >
        <el-button
          type="success"
    ...
    <el-dialog title="编辑" :visible.sync="updateDialogVisible" width="50%">
      <el-form
        ref="updateRouterFormRef"
        :model="updateRouterForm"
        label-width="70px"
      >
        <el-form-item label="ID" prop="routeId">
          <el-input
            v-model.number="updateRouterForm.routeId"
            disabled
          ></el-input>
        </el-form-item>
        <el-form-item label="父类ID" prop="parentId">
          <el-input
            v-model.number="updateRouterForm.parentId"
            disabled
          ></el-input>
        </el-form-item>
        <el-form-item label="路由名称" prop="routeName">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="updateDialogVisible = false" icon="el-icon-error"
          >取 消</el-button
        >
        <el-button type="success" @click="submitUpdateRoute" icon="el-icon-edit"
          >修 改</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    //*获取路由列表
    async getRouterList() {
      const { data: res } = await getRoute({
        parentId: this.Data.parentId,
      });
      if (res.code !== 200) return this.$message.error("获取用户路由失败~");
    ...
    async toNextLevel(id) {
      const { data: res } = await getRoute({ parentId: id });
      if (res.code !== 200)
        return this.$message.error("查询失败 Σ(っ °Д °;)っ");
    ...
    async deleteRouter(id) {
      const { data: res } = await deleteRoute({ routeId: id });
      if (res.code !== 200) {
    ...
    async submitAddRoute() {
      ...
      const { data: res } = await addRoute(this.addRouterForm);
    ...
    async submitUpdateRoute() {
      const { data: res } = await updateRoute(this.updateRouterForm);
