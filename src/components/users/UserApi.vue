  ...
  <div>
    ...
    <div class="top">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>用户Api</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <div class="card">
      <el-card>
        <el-row>
          <el-button type="success" round @click="showDialog">点击添加</el-button>
        </el-row>
        <el-table :data="UserApiList" border stripe>
    ...
    <!-- 添加新信息 -->
    <el-dialog title="添加用户api" :visible.sync="addDialogVisible" width="50%" @close="closeDialog">
        <!-- 内容主体区 -->
...
<script>
import { getUserApi,addUserApi,deleteUserApi,updateUserApi } from "@/api/users";
export default {
  data() {
    return {
  ...
  methods: {
    async getUserApiList() {
      const { data: res } = await getUserApi({ params: this.query });
      if (res.code !== 200) {
        return this.$message.error("获取用户Api失败~");
      }
      this.$message.success("获取用户Api成功！");
      this.UserApiList = res.data.data;
    ...
    //!删除用户api
    async deleteApi(id){
      const {data:res} = await deleteUserApi({apiId:id})
      console.log(res);
      if(res.code !== 200)
    ...
    //*更新用户api
    async updateApi(){
      const {data:res} = await updateUserApi(this.updateForm)
      if(res.code !== 200)
